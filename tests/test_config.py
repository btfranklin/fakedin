...
def test_validate_settings_raises_without_key(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(config.settings, "openai_api_key", "")
    with pytest.raises(ValueError):
        config.validate_settings()


def test_validate_settings_passes_with_key(monkeypatch: pytest.MonkeyPatch) -> None:
